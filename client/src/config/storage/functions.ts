...
import { updateChecksum } from "./utils.ts";

import { fileScalingManagerIdlFactory, fileStorageIdlFactory, scalingCanId, storageCanId } from "../../hooks/exporter";


const motoko_identity = Ed25519KeyIdentity.generate();
...
async function uploadChunk({ chunk, order, batch_id }: Chunk): Promise<number> {
  const res = await fileStorageActor.create_chunk(batch_id, chunk, BigInt(order));
  return Number(res);
}
...
export function uploadFile(file: File, path: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    asset_reader.onload = async () => {
      if (!(asset_reader.result instanceof ArrayBuffer)) {
        reject(new Error('FileReader result is null or not an ArrayBuffer.'));
        return;
      }
      ...
      for (let start = 0, index = 0; start < asset_unit8Array.length; start += chunkSize, index++) {
        ...
        promises.push(uploadChunk({ chunk, order: index , batch_id}));
      }

      Promise.all(promises).then(async (chunk_ids) => {
